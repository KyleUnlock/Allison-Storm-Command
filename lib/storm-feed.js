...
async function zipCentroid(zip, deps = {}) {
  ...
  // A resolved centroid is effectively permanent (30d). A miss (null) is likely
  // a transient zippopotam.us blip on a valid ZIP — cache it only briefly so one
  // ~4s outage cannot blackhole live hail matching for that ZIP for a month
  // (mirrors recentHailNear's short degraded-miss TTL).
  const ttl = centroid ? DEFAULTS.centroidTtlMs : Math.min(DEFAULTS.hailTtlMs, 15 * 60 * 1000);
  await cacheSet(store, key, centroid, ttl, now);
  return centroid;
}
