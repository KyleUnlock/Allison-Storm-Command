...
async function readRawBody(req) {
  ...
  return new Promise((resolve) => {
    ...
    // req.destroy() (1MB guard) emits 'close' without 'end'/'error'; resolve the
    // truncated buffer so the awaited Promise settles. A truncated body still
    // fails the HMAC closed, so no fence is loosened. Registered last.
