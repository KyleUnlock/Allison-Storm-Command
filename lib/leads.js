 ...
 *
 * LOCKED billing rule: the platform fee is 20% of PROFIT (never revenue,
 * never 30%). A job bills only when ALL hold:
...
async function createLead(input = {}, meta = {}) {
  ...
  await ledger.append({
    ...
    data: { source: lead.source, attribution: lead.attribution.token },
  });
  return lead;
}
